...
**协程在挂起（Suspend）时，控制权究竟交还给谁？**

控制传递机制：对称（Symmetric） v.s. 非对称（Asymmetric）协程
- 非对称协程：具有明显的调用者（Caller）和被调用者（Callee）的关系。它的控制流转移操作是“非对称”的。（类似函数调用）
  - 控制机制：提供两种操作：resume（恢复）和 yield（挂起）。
- 对称协程：所有的协程都是平等的（Peer-to-Peer），没有上下级的调用关系。（可以任意跳转，类似goto语句）
  - 提供一种统一的转移操作：一个协程在挂起自己的同时，必须显式指定下一个要运行的目标协程
