...
![w:750](figs/addr-space-os-detail.png)
...
---
**陷入上下文页**
- 跳板页的``_all_traps``汇编函数会保存相关寄存器
- 跳板页的``_restore``汇编函数会恢复相关寄存器
...
---
使能页机制的OS
- 只通过``sscratch``寄存器进行中转**栈指针**和**页表基址**
- 产生异常后，通过``sscratch``进行usr sp<->Trap 上下文地址;
- 保存用户态寄存器到Trap 上下文;
- 读出位Trap 上下文中的页表基址/应用的内核栈指针/**trap_handler**地址；
...
- **地址空间的抽象**
  - 应用感知虚存 
![bg right:65% 100%](figs/app-as-full.png)
...
- **地址空间的抽象**
  - 建立&感知虚拟/物理地址
  - 在虚拟地址空间之间穿越
![bg right:40% 100%](figs/kernel-as-low.png)
---
### 内核程序设计
...
![bg right:50% 100%](figs/kernel-as-high.png)
...
- 这个运行的程序对代码和数据的直接访问范围限制在它关联的虚拟地址空间之内
- 任务的地址空间，内核的地址空间
...
- **地址空间** ``MemorySet``的相关操作
  - 在地址空间中插入/删除一个逻辑段
    - 需要更新页表中的相应页表项
    - 更新逻辑段对应的物理页帧内容 
![bg right 100%](figs/kernel-as-low.png)


---
### 内核程序设计 -- 实现地址空间抽象 -- 应用地址空间
...
- Q:为何不直接把 Trap 上下文仍放到应用的内核栈中呢？
- A:这样就要求地址空间的切换不能影响指令的连续执行，即要求应用和内核地址空间在切换地址空间指令附近是平滑的。
