...
- 紧急等待队列：唤醒使用的紧急队列。因为线程会执行唤醒操作，所以可能有多个等待使用管程的队列，这样队列称为紧急队列，它的优先级高于等待队列（为互斥而等待的队列）
...
---
### 管程 -- 条件变量
- 管程中的共享变量在管程外部是不可见的，外部只能通过调用管程中所说明的成员过程 (函数）来间接地访问管程中的共享变量
  - 规定管程互斥进入；
  - 管程中设有等待队以及相应的等待及唤醒操作。
...
- leave过程：当线程离开管程时，如果紧急队列不为空，唤醒紧急队列中的线程，并将所持锁赋予该线程；如果为空，释放lock，唤醒入口等待队列某个线程；
...
<!-- https://www.cnblogs.com/upnote/p/13030741.html   Java synchronized的理论基础-管程(Monitor) -->
  - 1. T1 进入 管程monitor
  - 2. T1 等待资源 (进入wait queue)
  - 3. T2 进入monitor
  - 4. T2 资源可用 ，通知T1恢复执行，并把自己转移到signal queue
  - 5. T1 重新进入 monitor
  - 6. T1 离开monitor
  - 7. T2 重新进入 monitor
  - 8. T2 离开monitor
...
<!-- https://www.cnblogs.com/upnote/p/13030741.html   Java synchronized的理论基础-管程(Monitor) -->
  ...
  - 2. T1 等待资源r
  - 3. T2 进入monitor
  - 4. T2 离开Monitor,并给通知等待资源r的线程，资源可用
  - 5. T1 重新进入 monitor
  - 6. T1 离开monitor
