...
![bg right:60% 100%](figs/os-history.png)
...
操作系统 | 功能      | 系统调用个数 
---------|----------|----------
 LibOS | APP与HW隔离，简化应用访问硬件的难度和复杂性 | 0
 BatchOS | APP与OS隔离，加强系统安全，提高执行效率 | 2
...
SyncMutex OS | 支持共享资源的多线程同步互斥访问 | 27
Device OS | 支持网络/图形/键盘等外设和基本网络协议栈 | 34
...
![bg right:65% 95%](figs/os-history.png)
<!-- ---
### 课程建设的具体实施
**30个系统调用**
```rust
...
![bg right:35% 90%](./figs/online-os.png)
...
- [春秋季在线slides](https://learningos.github.io/os-lectures/)：包括13讲课件、思维导图、实验介绍、本科毕设
- [在线实验代码](https://github.com/rcore-os/rCore-Tutorial-v3)：分Rust和C两个版本
- [在线实验指导](https://github.com/rcore-os/rCore-Tutorial-Book-v3)：分Rust和C两个版本
- [开源操作系统训练营]( https://github.com/LearningOS/rust-based-os-comp2024)：一年两期：春夏季学期、秋冬季学期
...
---
### Rust语言相关设计
**所有权（Ownership）**: 在Rust中，所有权系统确保了值的移动和借用规则，避免数据竞争和空悬指针问题。你可以将另一个数据结构的所有权直接转移到当前结构中。
...
---
### Rust语言相关设计
**引用（References）**: 如果你不想转移所有权，可以使用引用。引用允许你借用另一个数据结构而不是拥有它。引用分为可变引用和不可变引用。
...
---
### Rust语言相关设计
**智能指针（Smart Pointers）**: Rust提供了多种智能指针，如Box, Rc, Arc等，这些智能指针封装了原始指针，提供了所有权和借用的自动管理。

- Box<T>: 一个简单的智能指针，用于在堆上分配数据。
- Rc<T>: 一个引用计数的智能指针，允许多个所有者。
- Arc<T>: Rc<T>的线程安全版本，用于多线程环境。
...
---
### Rust语言相关设计
特征对象（Trait Objects）: 如果你需要在运行时动态地处理不同类型的数据结构，可以使用特征对象。这通常涉及到使用dyn关键字和特征（traits）。
```rust
trait DataTrait {
