...
- mcause(Machine Exception Cause)它指示发生异常的种类。
  ...
  - SPP表示变化之前的特权级别是S-Mode还是U-Mode，MPP表示变化之前是S-Mode还是U-Mode还是M-Mode，
...
##  M-Mode编程 -- 异常/中断处理的控制权移交
异常/中断委托寄存器
* mideleg (Machine Interrupt Delegation）CSR 控制将哪些中断/异常委托给 S-Mode处理
...
## M-Mode编程 -- 异常/中断处理的控制权移交
异常/中断委托寄存器
* medeleg (Machine Exception Delegation）CSR 控制将哪些中断/异常委托给 S-Mode处理
...
---
##  S-Mode编程 -- 虚存机制
- S、U-Mode中的虚拟地址会以从根部遍历页表的方式转换为物理地址：
...
---
## RISC-V 系统编程：S-Mode的虚拟内存系统

- 虚拟地址将内存划分为固定大小的页来进行地址转换和内容保护。
- satp（Supervisor Address Translation and Protection，监管者地址转换和保护）S-Mode控制状态寄存器控制了分页。satp 有三个域：
