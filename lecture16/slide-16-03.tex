...
Ref: \href{http://ermak.cs.nstu.ru/understanding.linux.kernel.pdf }{Understanding the Linux Kernel}
...
%    \framesubtitle{xxxx}
    \begin{itemize}
        \item Application registers handlers with signal() or sigaction()
...
%    \framesubtitle{xxxx}
    \begin{itemize}
        \item Signals are the underlying mechanism for Exceptions and catch blocks
        \item JVM or other runtime system sets signal handlers
...
A small piece of assembly code to perform cleanup after handling the signal.
    \begin{itemize}
        \item Signal trampoline code calls sigreturn().
        \item sigreturn() undoes everything that was done in order to invoke the signal handler
        \begin{itemize}
            \item Changing the process's signal mask, switching signal stacks (see sigaltstack(2))
...
%    \framesubtitle{xxxx}
    \begin{itemize}
        \item The first available thread gets the signal
        \item Most handlers run on the thread's stack.
        \item A handler can run on an alternate stack
...
%    \framesubtitle{xxxx}
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item These execute inside the kernel    
        \end{itemize}
...
%    \framesubtitle{xxxx}
    \begin{itemize}
        ...
        \begin{itemize}
            \item And make runnable if blocked with TASK_INTERRUPTIBLE flag
        \end{itemize}
